...
trait EncoderInstances:
  ...
  given Encoder[String] = Encoder.fromFunction(s => Json.Str(s.toString))
    // TODO Implement the `Encoder[String]` instance

  /** An encoder for `Boolean` values */
  given Encoder[Boolean] = Encoder.fromFunction(b => Json.Bool(b))
...
trait DecoderInstances:
  ...
  given Decoder[String] = Decoder.fromPartialFunction { case Json.Str(s) => s}

  /** A decoder for `Boolean` values */
  // TODO Define a given value of type `Decoder[Boolean]`
  given Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(b) => b}
  /**
  ...
  Decoder.fromPartialFunction {
    case Json.Arr(arr) =>
      arr.map(decoder.decode).withFilter(_.isDefined).map(_.get)
  }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(obj) => obj.get(name) match {
        case Some(property) => decoder.decode(property)
         case _ => None
      }
      case _ => None
    }
...
trait PersonCodecs:
  ...
  Decoder.field[String]("name")
    .zip(Decoder.field[Int]("age")).transform[Person](
